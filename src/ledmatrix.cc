 ...
 RGBMatrix::Options options = RGBMatrix::Options();
	...
	if(args.Length() > 0 && args[0]->IsObject()) {
		//Set options props from passed object.
		 ...
		 v8::Local<v8::Context> context = v8::ContextFromHeapObject(v8::Utils::OpenHandle(object));

		 if (object->HasOwnProperty(context, Nan::New("rows").ToLocalChecked()) == v8::Just(true)) {
